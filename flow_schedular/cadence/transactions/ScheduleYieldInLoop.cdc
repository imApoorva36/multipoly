...
/// Schedule a looping yield generation with a delay between each execution
transaction(
    delaySeconds: UFix64,
    priority: UInt8,
    ...
    prepare(signer: auth(BorrowValue, IssueStorageCapabilityController, SaveValue, GetStorageCapabilityController, PublishCapability) &Account) {
        ...
        if let cap = signer.capabilities.storage
                            .getControllers(forPath: /storage/YieldTokenLoopTransactionHandler)[0]
                            .capability as? Capability<auth(FlowTransactionScheduler.Execute) &{FlowTransactionScheduler.TransactionHandler}> {
            handlerCap = cap
        } else {
            handlerCap = signer.capabilities.storage
                            .getControllers(forPath: /storage/YieldTokenLoopTransactionHandler)[1]
                            .capability as! Capability<auth(FlowTransactionScheduler.Execute) &{FlowTransactionScheduler.TransactionHandler}>
        }
        ...
        let loopConfig = YieldTokenLoopTransactionHandler.createLoopConfig(
            delay: delaySeconds,
        ...
        log("Scheduled yield loop transaction id: ".concat(transactionId.toString()).concat(" at ").concat(future.toString()).concat(" with delay: ").concat(delaySeconds.toString()))
    }
}